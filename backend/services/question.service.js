...
exports.getQuestion = async function (id) {
  try {
    // Find the Data
    var _details = await Question.findOne({ _id: id }).populate({
      path: "section_id",
      select: "name",
    });
    if (_details._id) {
...
exports.getQuestionOne = async function (id) {
  try {
    // Find the Data
    var question = await Question.findOne({ _id: id }).populate({
      path: "section_id",
      select: "name",
    });
...
exports.getGroupedQuestionsByAssessment = async function (assessmentId) {
  try {
    const sections = await Section.aggregate([
      ...
      {
        $addFields: {
          // Sorting questions by the order field
          questions: {
            $map: {
              input: {
                $sortArray: { input: "$questions", sortBy: { order: 1 } },
              },
              as: "q",
              in: "$$q",
            },
                      ...
                      "$$question",
                      {
                        child_questions: {
                          $filter: {
                            ...
                            cond: {
                              $eq: [
                                "$$sub_question.parent_question_id",
                                "$$question._id",
                              ],
                            },
                          },
      ...
      {
        $sort: {
          order: 1, // Sort sections based on their 'order' field in ascending order
        },
      },
      {
        $project: {
    ...
    throw new Error(
      `Error occurred while grouping questions: ${error.message}`
    );
  }
};
...
//         $unwind: {
//           path: "$questions",
//           preserveNullAndEmptyArrays: true, // Optional: Include sections without questions
//         },
//       },
//       {
//         $match: {
//           "questions.deletedAt": null, // Ensure questions are not deleted
//         },
//       },
//       {
//         $group: {
//           _id: "$_id",
//           section_id: { $first: "$_id" },
//           name: { $first: "$name" },
//           description: { $first: "$description" },
//           order: { $first: "$order" },
//           status: { $first: "$status" },
//           questions: { $push: "$questions" }, // Collect questions into an array
//         },
//       },
//       {
//         $sort: {
...
exports.getSectionsWithQuestionsAndAnswers = async function (
  assessmentId,
  assessmentReportId
) {
  try {
                ...
                $expr: {
                  $and: [
                    { $eq: ["$section_id", "$$sectionId"] },
                    {
                      $eq: [
                        "$assessment_id",
                        mongoose.Types.ObjectId(assessmentId),
                      ],
                    },
                    { $eq: ["$status", 1] },
                    { $eq: ["$deletedAt", null] },
                      ...
                      $expr: {
                        $and: [
                          { $eq: ["$question_id", "$$questionId"] },
                          {
                            $eq: [
                              "$asessment_report_id",
                              mongoose.Types.ObjectId(assessmentReportId),
                            ],
                          },
                        ],
                      },
              ...
              $match: {
                $expr: {
                  $and: [
                    ...
                    { $eq: ["$status", 1] },
                    { $eq: ["$deletedAt", null] },
                    { $ne: ["$parent_question_id", null] }, // Select child questions
                  ],
                },
              },
            },
        ...
        result.sections.forEach((section) => {
          if (section.questions) {
            section.questions.forEach((question) => {
              ...
              const matchingChildQuestions = childQuestions.find(
                (child) => child._id.toString() === question._id.toString()
              );
...
// ) {
//   try {
//     // Step 1: Match all sections with their questions and answers
//     const sections = await Section.aggregate([
//       {
//         $sort: { order: 1 },
//       },
...
//           status: 1,
//           deletedAt: null,
//         }, // Match sections by assessment_id
//       },
//       {
//         $lookup: {
//           from: "assessments", // Name of the assessments collection
//           localField: "assessment_id",
//           foreignField: "_id",
//           as: "assessmentDetails", // Add assessment details
//         },
//       },
//       {
//         $unwind: "$assessmentDetails", // Unwind the assessment details
//       },
//       {
//         $lookup: {
//           from: "questions", // Name of the questions collection
//           let: { sectionId: "$_id" }, // Current section ID
//           pipeline: [
//             {
//               $match: {
//                 $expr: {
//                   $and: [
//                     { $eq: ["$section_id", "$$sectionId"] }, // Match section_id
//                     {
//                       $eq: [
//                         "$assessment_id",
//                         mongoose.Types.ObjectId(assessmentId),
