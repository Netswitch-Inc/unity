...
const EditOpenVASScanReport = () => {
    ...
    const [openVASScanReportItem, setOpenVASScanReportItem] = useState(store.openVASScanReportItem || initOpenVASScanReportItem,);
    ...
    useEffect(() => {
        ...
        if (store?.actionFlag === "OVSR_SCH_ITM" || store?.actionFlag === "OVSR_SCH_ITM_ERR") {
    ...
    }, [store.error, store.success, store.actionFlag, store.openVASScanReportItem, navigate, dispatch])
    ...
    <Field name="timestamp">
      {({ field, form }) => {
        const formatDateTimeLocal = (date) => {
          ...
          return d.toISOString().slice(0, 16); // Convert to "YYYY-MM-DDTHH:MM" format
        };
        ...
        return (
          <Input
            ...
            onChange={(e) => form.setFieldValue("timestamp", e.target.value)}
            className="mb-0"
          />
        );
      }}
