...
async function getCISListRequest() {
  return instance
    .get(`${API_ENDPOINTS.Cis.listingAll}`)
...
async function getComplinceSubcontrol(params) {
  return instance
    .get(`${API_ENDPOINTS.Cis.listingSubcontrols}`, { params })
    .then((items) => items.data)
    .catch((error) => ({ error }));
}

// Thunk for fetching CIS list
export const getComplinceSubcontrolListing = createAsyncThunk(
  "appCis/getCisSubcontrolsListing",
  async (params) => {
    try {
...
async function downloadFromStorageRequest(params, onProgress) {
  return instance
    .get(`${API_ENDPOINTS.Cis.download}`, {
      params,
      responseType: "blob",
...
export const downloadFromStorage = createAsyncThunk(
  "appCis/Download",
