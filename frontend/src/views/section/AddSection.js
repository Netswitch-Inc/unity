...
import { createSection } from "./store";
...
const AddSection = () => {
  const initialValues = { name: "", description: "", order: "", status: 0 };
  ...
  useEffect(() => {
    if (store.actionFlag === "SECTION_CREATED" && store.success) {
  ...
  useEffect(() => {
    if (store.actionFlag === "SECTION_CREATED" && showSnackBar) {
      setTimeout(() => {
          ...
          <Col>
            <Card>
              <div className="p-0 border-bottom pb-2 card-header row justify-content-between m-0">
                <h3 className="card-title mb-0 mt-0">Add Section</h3>
                <button
                  type="button"
