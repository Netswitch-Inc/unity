...
import { initialProject, priority, superAdminRole, projectsPermissionId, governanceGroupPermissionId } from "utility/reduxConstant";
...
const AddProject = () => {
    ...
    useEffect(() => {
        if (data) {
            setInitialProjectValue((prev) => ({
                ...
                framework_id: data.framework_id ? [{ label: data.framework_id.label, value: data.framework_id._id }] : []
            }));
        }
    }, [data]);
    ...
    const validationSchema = Yup.object().shape({
        ...
        affected_scope: Yup.string()
            .required('Affected scope is required'),
    ...
    useEffect(() => {
        ...
        if (store?.actionFlag === 'PROJECT_CREATED_ERROR') {
            setshowSnackbar(true);
    ...
    useEffect(() => {
        if (store?.actionFlag === 'PROJECT_CREATED_SUCCESS' && showSnackBar) {
            setTimeout(() => {
        ...
        if (store?.actionFlag === 'PROJECT_CREATED_ERROR' && showSnackBar) {
            setTimeout(() => {
                setshowSnackbar(false);
                setSnakbarMessage("");
            }, 2000);
                                    ...
                                    {({ setFieldValue, values, errors, isSubmitting }) => (
                                        <Form>
                                            <Row>
                                                ...
                                                {companiesList?.length > 0 && <Col md={6}>
                                                    <FormGroup>
                                                        <label>
                                                            Location
                                                        </label>
